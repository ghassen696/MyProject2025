...
from pyspark.sql.functions import (
    ...
    lead,struct,array_contains,expr,to_utc_timestamp, date_format
)
...
from pyspark.sql.functions import to_timestamp, date_format, unix_timestamp, from_unixtime

# Add date with explicit UTC handling
daily_df = parsed_df.withColumn(
    "date_str",
    date_format(col("timestamp"), "yyyy-MM-dd")
).withColumn(
    "date",
    unix_timestamp(col("date_str"), "yyyy-MM-dd") * 1000  # Convert to milliseconds
).drop("date_str")
# ------------------- Time Spent per App -------------------
...
heartbeat_df = daily_df.filter(col("event") == "heartbeat") \
    .groupBy("employee_id") \
...
"""try:
    while True:
        now=datetime.now()
        if now.hour >=23:
            logger.info("Stopping job because it reached 18:00")
            daily_query.stop()
            spark.stop()
            break
        time.sleep(60)
