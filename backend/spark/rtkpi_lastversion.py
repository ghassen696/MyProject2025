...
from pyspark.sql.functions import (
    ...
    lead, struct, expr, unix_timestamp, element_at
)
...
activity_events = [
    "keystrokes", "window_switch", "clipboard_paste", "shortcut", 
    "pause", "resume", "idle_start", "idle_end", "heartbeat"
]
...
# ------------------- Time Spent per App -------------------
app_window = Window.partitionBy("employee_id", "date", "application").orderBy("timestamp")

app_usage = daily_df \
    .withColumn("next_ts", lead("timestamp").over(app_window)) \
    .withColumn(
        "duration_sec", 
        when(col("next_ts").isNotNull(), 
             col("next_ts").cast("long") - col("timestamp").cast("long")
        ).otherwise(0)
    ) \
...
daily_agg = daily_df.groupBy("employee_id", "date").agg(
    ...
    # FIX: idle_duration_sec is already in seconds, don't need conversion
    sum_(when(col("event") == "idle_end", col("idle_duration_sec")).otherwise(0)).alias("total_idle_sec"),
    count(when(col("event") == "pause", 1)).alias("pauses_today"),
    # FIX: duration_minutes is already in minutes
...
# ------------------- Compute KPIs -------------------
# FIX: Calculate active minutes correctly
daily_agg = daily_agg.withColumn(
    "total_session_minutes",
    (col("last_event_time").cast("long") - col("first_event_time").cast("long")) / 60.0
).withColumn(
    "total_idle_minutes",
    col("total_idle_sec") / 60.0
).withColumn(
    "active_minutes",
    when(
        col("total_session_minutes") - col("total_idle_minutes") - col("total_pause_minutes_today") > 0,
        col("total_session_minutes") - col("total_idle_minutes") - col("total_pause_minutes_today")
    ).otherwise(0)
)

# FIX: Keystrokes per minute calculation
daily_agg = daily_agg.withColumn(
    "keystrokes_per_minute",
    when(col("active_minutes") > 0, col("keystrokes_today") / col("active_minutes")).otherwise(0)
)

# ------------------- Last Idle Duration -------------------
# Extract last idle event's duration
daily_agg = daily_agg.withColumn(
    "last_idle_event",
    expr("""
        filter(events_today, x -> x.event IN ('idle_start', 'idle_end'))
    """)
).withColumn(
    "last_idle_duration",
    expr("""
        CASE 
            WHEN size(last_idle_event) > 0 THEN 
                element_at(last_idle_event, -1).idle_duration_sec / 60.0
            ELSE 0 
        END
    """)
).drop("last_idle_event")
...
daily_agg = daily_agg.withColumn(
    ...
    "last_event_time",
    col("last_event.timestamp")
).withColumn(
    "employee_status",
    when(col("last_event.event") == "pause", lit("pause"))
    .when(col("last_event.event").isin("idle_start", "idle_end"), lit("idle"))
    .when(col("last_event.event").isin(
        "keystrokes", "window_switch", "clipboard_paste", "shortcut", "resume", "heartbeat"
    ), lit("active"))
...
# ------------------- Productivity Score -------------------
# Simple productivity score: (keystrokes_per_minute / 100) * 100, capped at 100
daily_agg = daily_agg.withColumn(
    "productivity_score",
    when(col("keystrokes_per_minute") > 0,
         when(col("keystrokes_per_minute") >= 100, lit(100))
         .otherwise(col("keystrokes_per_minute"))
    ).otherwise(0)
)
...
heartbeat_with_email = heartbeat_with_email.withColumn(
    ...
    when(col("last_heartbeat").isNotNull(),
         unix_timestamp(current_timestamp()) - unix_timestamp(col("last_heartbeat"))
    ).otherwise(999999)
)

# Filter employees missing heartbeat for more than 2 minutes (120 seconds)
...
# Optional: Uncomment to enable email alerts
"""
def notify_missing_heartbeat(batch_df, batch_id):
    ...
    filtered = updated_df.filter(col("seconds_since_last_heartbeat") > 120)
...
daily_agg = daily_agg.withColumnRenamed("total_idle_sec", "total_idle_today")
