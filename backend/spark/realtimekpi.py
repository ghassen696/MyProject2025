...
"""#--------------------heartbeat----------------------
# Get last heartbeat per employee
...
# Join with daily_agg to only include employees who gave consent
heartbeat_with_email = heartbeat_with_email.join(
    daily_agg.select("employee_id", "employee_worked"),
    on="employee_id",
    how="left"
).filter(col("employee_worked") == True)
...
# Filter employees who missed a heartbeat (>5 minutes)
missing_heartbeat_df = heartbeat_with_email.filter(col("seconds_since_last_heartbeat") > 300)
...
def send_alert_email(email, employee_id, last_heartbeat):
    ...
    msg['Subject'] = f"ALERT: Missing Heartbeat for {employee_id}"
    ...
    try:
        with smtplib.SMTP("smtp.company.com", 587) as server:
            server.starttls()
            server.login("huaweitesttest1@gmail.com", "your_app_password_here")
            server.send_message(msg)
        logger.info(f"Alert sent for employee {employee_id} to {email}")
    except Exception as e:
        logger.error(f"Failed to send email to {email}: {e}")

# ------------------- foreachBatch Function -------------------
def notify_missing_heartbeat(batch_df, batch_id):
    records = batch_df.collect()  # collect small batch
    for row in records:
        if row.email:  # only send if email exists
            send_alert_email(row.email, row.employee_id, row.last_heartbeat)
        else:
            logger.warning(f"No email for employee {row.employee_id}, cannot send heartbeat alert.")
